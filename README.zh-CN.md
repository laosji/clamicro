...
**先说清楚默认行为，免得你以为它在拦一切：普通操作等你 10 秒，没人管就自动通过。** 真正会一直等你的是高风险操作（`rm -rf`、force push、动 `~/.ssh` 之类），它们会等到超时然后**自动拒绝**。也就是说默认配置下，日常操作它是「告诉你」，高危操作它才是「拦住你」。这个 10 秒可以在设置里改，改成 0 就是每一条都等你。
...
代价要说清楚：**离开电脑后没有任何东西会叫你**，高风险操作会等到超时（默认 570 秒）被自动拒绝，那一轮任务失败。这是有意的默认——人不在就不该放行 `rm -rf`。想主动看的话，手机浏览器随时能打开看板。
