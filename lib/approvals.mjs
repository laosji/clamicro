...
export class ApprovalStore extends EventEmitter {
  ...
  create({ session_id, tool_name, tool_input, permission_rule, tool_use_id, cwd }, policy = {}) {
    ...
    const ap = {
      ...
      status: 'pending', // pending | allowed | auto_allowed | denied | expired | abandoned | superseded
  ...
  /**
   * 同一个 tool_use_id 的工具已经开始执行了 —— 说明这次授权在别处
   * （终端弹框、权限规则、auto 模式）已经被批准，而我们这条还挂着空等。
   *
   * Claude Code 不会取消已经发出的 PermissionRequest 请求，所以连接一直开着、
   * req 的 close 事件也不触发，记录会一直显示「待审批」直到超时。
   * 界面上就出现了「明明已经执行完，却还让你去批」的幽灵条目。
   *
   * 用 tool_use_id 对上就能识破。返回 allow —— 操作事实上已经在跑了，
   * 这时候回 deny 才是与现实矛盾的。
   */
  supersede(toolUseId) {
    if (!toolUseId) return null
    for (const ap of this.#items.values()) {
      if (ap.status === 'pending' && ap.tool_use_id === toolUseId) {
        this.#settle(ap.id, 'superseded', 'elsewhere')
        return ap
      }
    }
    return null
