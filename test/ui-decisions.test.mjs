...
test('§6.5 这一节还在', () => {
  ...
  for (const h of ['三格导航', '「等我回话」：把 Stop 挂住', '否决过的（别再做回去）',
    '改 UI 时问自己的三个问题']) {
...
test('「等我回话」那四档文案，一档都不能少', async (t) => {
  /*
   * 四档说的是四件不同的事，而**第四档是唯一会骗人的地方**：会话已经停了
   * 的时候打开开关，对眼下这一次没有用（Stop 已经放行，挂不住一个走掉的
   * 请求），可你恰恰是因为「它在等你回话」才去打开它的。
   *
   * 少掉那一档不会报错，只会让这个功能在最需要说实话的时刻沉默。
   */
  const code = codeOnly(read('ui/session.html'))
  const 档 = [
    ['不会立刻送达', '关着、在跑'],
    ['跑完这一轮会挂住', '开着、在跑'],
    ['现在发就立刻接着跑', '开着、正挂着——唯一「立刻」成立的时刻'],
    ['但这一轮已经停了', '开着、会话已停——唯一会骗人的那一档'],
  ]
  for (const [needle, which] of 档) {
    await t.test(which, () => {
      assert.ok(code.includes(needle),
        `找不到「${needle}」——「${which}」这一档没了。四档说的是四件不同的事`)
    })
  }

  await t.test('倒数用服务端给的时间戳，不是页面自己猜的', () => {
    // 手机什么时候打开的、SSE 那条消息路上花了多久，页面都不知道
    assert.match(code, /handoff[\s\S]{0,400}?\.since|h\.since/,
      '倒数没用服务端给的 since——那个秒数就是编的')
  })
})
