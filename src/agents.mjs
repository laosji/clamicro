...
export const AGENTS = {
  'claude-code': {
    ...
    /**
     * 说的是**它实际做什么**，不是我们希望它做什么。
     *
     * 「取消本轮」这个名字曾经承诺了一件做不到的事：它拦的是**下一次工具
     * 调用**，而一个不调工具的回合（长文本输出、规划、纯思考）它碰不到
     * ——架构文档 §5 写着「单靠 PreToolUse 拦不住一个不调工具的回合」。
     */
    cancelNote: '在下一次工具调用前拦住它。当前这一步仍会跑完；若这一轮不再调用工具，则不会触发。',
  },
