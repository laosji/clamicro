...
export function pageRoutes(ctx) {
  ...
  return async function handlePages(req, res, url, path) {
    ...
    /**
     * 新手引导。现在只有**一屏**（原来是三屏三路由）。
     *
     * 1|2|3 三个路由全部保留、全部渲染同一屏：升级上来的人 localStorage 里
     * 可能还留着 `ccm-onboard-step=2/3`，首页会照着它重定向过来——改成 404
     * 就等于把这批人送进一个打不开的页面。那个 key 由引导页自己清掉。
     *
     * 必须已登录：这一屏讲的是「你现在能做什么」，对还没配对的人毫无意义，
     * 而且那颗按钮会真的创建一条审批。未登录一律回配对页。
     */
