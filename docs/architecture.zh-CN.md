...
### 引导 `ui/onboarding.html`

**一屏。** 配对成功后落在这里，上面只有一件事：发一条测试审批。

| 部位 | 现在的形状 | 判据 |
|---|---|---|
| 顶部 | `● 已连接到 <Mac 名>` | 唯一的作用是让你确认「连的是我那台电脑」；没有名字它就只是一句正确的废话 |
| 能力清单 | 三条小字（看命令 / 允许拒绝 / 发一句话） | 参考信息，不占一次点击 |
| 主按钮 | 「发一条测试审批」 | 这一步同时是**验收**——批准或拒绝都不执行任何命令 |
| 失败 | 不标记 `ccm-onboarded` | 标记完成会让人下次直接落到一个同样坏掉的首页上，还少了这条线索 |

路由 `/ui/onboarding/1|2|3` **三个都保留、都渲染这一屏**：老版本在 localStorage
里留下的 `ccm-onboard-step` 可能是 2 或 3，改成 404 会把升级上来的人送进一个
打不开的页面。那个 key 由引导页自己清掉。
...
**它解决的是哪一刻**：Claude Code 在终端里等你打字的那一刻，服务手上
**没有任何能回的请求**——Stop 早就回过 `{}` 了，之后那条
`Notification: idle_prompt` 是单向的，Claude Code 不等它的回包。所以那时你在
手机上打的字没有东西可以搭载，只能躺到你回终端敲一下。

开关做的事是**提前一步**：趁这一轮还没结束就说好「结束时挂住等我」，
于是 Stop 到了不立刻放行，挂 90 秒等你（`Inbox.HOLD_MS`）。手法和 pause
完全一样（§5 挂住 PreToolUse），**架构不变**——仍然是「只回答，不外发」。

那行送达提示有四档，说的是四件不同的事：

| 状态 | 那行字 | |
|---|---|---|
| 关着、在跑 | 不会立刻送达 · 跑完当前这一轮后进入对话 | 老样子 |
| 开着、在跑 | 跑完这一轮会挂住 90 秒等你 | 承诺发生在未来 |
| 开着、正挂着 | 正在等你 · 现在发就立刻接着跑（还有 N 秒） | 唯一「立刻」成立的时刻 |
| **开着、会话已停** | **已开，但这一轮已经停了 —— 要下一轮跑完才会等你** | **见下** |

**第四档是这个功能唯一会骗人的地方。** 会话已经停了的时候打开它，对眼下
这一次没有用：那一轮的 Stop 已经放行了，挂不住一个已经走掉的请求。而你
恰恰是**因为「它在等你回话」才去打开开关的**——那正是它帮不上忙的那一刻。
所以这一档单独一句话，而且是警告色。

三条约束：

1. **必须由人显式打开，不能靠猜。** 挂着 Stop 的时候终端不把提示符还给你，
   人在 Mac 前面看到的是「它好像还在跑」。靠「手机上开着这个页面」去猜的话，
   页面摔口袋里走开是常态，而猜错的代价是你自己的终端被堵住。谁打开的谁承担。
2. **超时自动关掉。** 挂满一次都没等到回话，说明你已经走开了。不关的话下
   一轮又堵一次，一直堵到你想起来为止——「忘了关」的代价应该是一次，
   不是每一轮。等到了话则**不关**，否则每回一句都要重开。
3. **90 秒不是技术上限。** hook 系统超时给到 570 秒（§src/limits.mjs），
   这里主动取小得多，因为超出的每一秒都花在堵住你自己的终端上。

倒数用的是服务端给的 `holdingSince`，不是页面自己猜的——手机什么时候打开的、
SSE 那条消息路上花了多久，页面都不知道，猜出来的秒数就是编的。
...
| 引导分三屏（连上了 / 你可以在手机上 / 先体验一条假请求） | 前两屏各占一次点击，说的是一行确认和一张清单；第三屏才是有价值的那一步，而它同时是验收。**在人最可能中途走开的那一刻，少两次「继续」比多两屏说明值钱**。路由留着，内容合并 |
| 安装流程里问「要不要接 DSH / Codex」 | 那两问排在 `closePrompt()` 之后，stdin 已经关了——`confirm()` 会把问题打出来、当场自己答「是」，然后去改别人家的配置，而 `optIn=true` 的全部意义就是不许代答。现在 install 只提一句，问答挪到 `npx clamicro connect dsh\|codex` |
